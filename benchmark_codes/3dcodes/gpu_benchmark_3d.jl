...
for m in ["Hydrostatic", "Nonhydrostatic", "ShallowWater", "ShallowWaterConservative"]
        for g in gridSizes
                ...
                if (m == "Nonhydrostatic")
                        grid = RectilinearGrid(GPU(), size=(g, g, g), x=(0, 2 * 3.14), y=(0, 2 * 3.14), z=(0, 2 * 3.14), topology=(Periodic, Periodic, Bounded))
                ...
                elseif (m == "ShallowWater")
                        grid = RectilinearGrid(GPU(), size=(g, g, g), x=(0, 2 * 3.14), y=(0, 2 * 3.14), topology=(Periodic, Periodic, Bounded), halo=(3, 3))
